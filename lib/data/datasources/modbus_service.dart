...
class ModbusService {
  ...
  Future<void> connect(String portName) async {
    ...
    while (retryCount < maxRetries) {
      ...
      try {
        ...
        log('Connection attempt ${retryCount + 1} failed: $e');
        retryCount++;
  ...
  Future<void> disconnect() async {
    if (_port != null) {
      try {
        ...
        log('Error disconnecting: $e'); // Swallow error on close
      }
      _port = null;
    }
  }
  ...
  void _send(Uint8List data) {
    if (_port != null) {
      try {
        if (!_port!.isOpen) {
          log('Warning: Port is closed, attempting to reopen...');
          if (!_port!.openReadWrite()) {
            log('Failed to reopen port.');
            return;
          }
        }
